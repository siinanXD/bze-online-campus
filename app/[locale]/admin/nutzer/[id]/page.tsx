...
export default async function NutzerDetailPage({
  ...
  return (
    <div className="space-y-4">
      <Card>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="font-mono text-sm text-fg-muted">{nutzer.benutzername}</p>
          </div>
        ...
        <dl className="mt-4 grid grid-cols-1 gap-x-6 gap-y-2 text-[15px] sm:grid-cols-2">
          <div>
            <dt className="text-xs text-fg-muted">{t('nutzer.spalte.rolle')}</dt>
            <dd className="font-semibold">{t(`rollen.${nutzer.rolle}`)}</dd>
          </div>
          <div>
            <dt className="text-xs text-fg-muted">{t('nutzer.feld.sprache')}</dt>
            <dd className="font-semibold">{nutzer.sprache}</dd>
          </div>
          <div>
            <dt className="text-xs text-fg-muted">{t('nutzer.letzterLogin')}</dt>
            <dd className="font-semibold">{formatiereDatum(nutzer.letzter_login)}</dd>
          </div>
          <div>
            <dt className="text-xs text-fg-muted">{t('nutzer.angelegtAm')}</dt>
            <dd className="font-semibold">{formatiereDatum(nutzer.created_at)}</dd>
          </div>
          {nutzer.rolle === 'teilnehmer' && (
            <div className="sm:col-span-2">
              <dt className="text-xs text-fg-muted">{t('nutzer.feld.kohorte')}</dt>
        ...
        {nutzer.muss_passwort_aendern && (
          <p className="mt-4 rounded-xl border border-border bg-bg px-3 py-2 text-sm text-fg-muted">
      ...
      <Card>
        <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-primary">{t('nutzer.aktionenTitel')}</h3>
        <Aktionen nutzerId={nutzer.id} aktiv={nutzer.aktiv} />
      </Card>

      {nutzer.rolle === 'teilnehmer' && (
        <Card>
          <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-primary">{t('nutzer.feld.kohorte')}</h3>
      ...
      {(nutzer.rolle === 'teilnehmer' || nutzer.rolle === 'ausbilder') && (
        <Card>
          <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-primary">{t('nutzer.zuweisungenTitel')}</h3>
