  ...
  return (
    <main className="mx-auto max-w-6xl space-y-4 p-4">
      ...
      {uebersichten.length === 0 ? (
        <Card><p className="text-sm text-muted">{t('keine')}</p></Card>
      ) : (
          ...
          {uebersichten.map((k) => (
            <Card key={k.kohorteId} className="space-y-2">
              ...
              <dl className="grid grid-cols-2 gap-2 text-sm">
                <div><dt className="text-muted">{t('teilnehmer')}</dt><dd className="font-semibold">{k.teilnehmerAnzahl}</dd></div>
                <div><dt className="text-muted">{t('fortschrittAvg')}</dt><dd className="font-semibold">{k.fortschrittAvg} %</dd></div>
                <div><dt className="text-muted">{t('lernpunkteAvg')}</dt><dd className="font-semibold">{k.lernpunkteAvg}</dd></div>
                <div><dt className="text-muted">{t('quoteAvg')}</dt><dd className="font-semibold">{k.quoteAvg} %</dd></div>
              </dl>
            </Card>
          ))}
        </div>
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wide text-accent">{t('schwachTitel')}</h2>
        {schwachStellen.length === 0 ? (
          <Card><p className="text-sm text-muted">{t('schwachKeine')}</p></Card>
        ) : (
            ...
            {schwachStellen.map((s) => (
              <div key={s.themaId} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
                <div>
                  ...
                  <p className="text-xs text-muted">{t('versuche', { n: s.versuche })}</p>
                </div>
