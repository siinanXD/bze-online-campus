...
export default async function TeilnehmerDetailPage({
  ...
  if (!data) {
    return (
      <main className="mx-auto max-w-4xl space-y-4 p-4">
        <Link href={`/${locale}/ausbilder`} className="touchable text-sm font-semibold text-primary underline-offset-4 hover:underline">
          {t('zurueck')}
        </Link>
        <Card><p className="text-fg-muted">{t('nichtGefunden')}</p></Card>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-4 p-4">
      <Link href={`/${locale}/ausbilder`} className="touchable inline-flex min-h-12 items-center text-sm font-semibold text-primary underline-offset-4 hover:underline">
        {t('zurueck')}
      </Link>
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-fg-muted">{profil.benutzername}</p>
        </div>
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wide text-primary">{t('bereiche')}</h2>
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wide text-primary">{t('fehlerschwerpunkte')}</h2>
        <Card className="divide-y divide-border">
          ...
          {data.themen.every((th) => th.offenFalsch === 0) && (
            <p className="text-sm text-fg-muted">{t('keineFehler')}</p>
          )}
        </Card>
      </section>

      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wide text-primary">{t('heatmap')}</h2>
        <Card>
          <div
          ...
          <p className="mt-2 text-xs text-fg-muted">{t('heatmapHinweis')}</p>
        </Card>
      </section>

      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wide text-primary">{t('pruefungen')}</h2>
        <Card className="divide-y divide-border">
          {data.pruefungen.length === 0 ? (
            <p className="text-sm text-fg-muted">{t('keinePruefungen')}</p>
          ) : (
      ...
      <section className="space-y-2">
        <h2 className="text-sm font-bold uppercase tracking-wide text-primary">{t('pruefungsreife')}</h2>
        <Card className="space-y-3 border-primary/30 bg-primary/5" role="note">
        ...
        {data.pruefungsreifen.length === 0 ? (
          <Card><p className="text-sm text-fg-muted">{t('keineReife')}</p></Card>
        ) : (
          data.pruefungsreifen.map((r) => (
            <Card key={r.id} className="space-y-3">
              ...
              {r.kommentar && (
                <p className="text-sm text-fg-muted">{t('kommentar')}: {r.kommentar}</p>
              )}
            </Card>
          ))
        )}
