  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="border-b border-border bg-surface px-4 py-3">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <Link href={`/${locale}`} className="min-w-0 text-start">
            ...
            <p className="truncate text-xs text-fg-muted">{t('app.traeger')}</p>
          </Link>
          <Link
            href={`/${locale}/login`}
            className="touchable inline-flex items-center justify-center rounded-xl bg-primary px-4 py-3 text-[15px] font-semibold text-fg-onPrimary transition hover:brightness-110"
          >
            {t('common.einloggen')}
          </Link>
        </div>
      ...
      <footer className="border-t border-border bg-surface px-4 py-6">
        <div className="mx-auto flex max-w-3xl flex-col gap-3 text-sm text-fg-muted sm:flex-row sm:items-center sm:justify-between">
