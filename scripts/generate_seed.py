...
# Träger
out.append(
    f"insert into traeger (id,name,slug) values ({q(TRAEGER)},{q('Berufsbildungszentrum Euskirchen')},{q('bze')}) on conflict (id) do nothing;"
)

# Kammer IHK Aachen
out.append(
    f"insert into kammern (id,name,typ,anschrift) values ({q(KAMMER)},{q('IHK Aachen')},'IHK',{q('Theaterstraße 6-10, 52062 Aachen')}) on conflict (id) do nothing;"
)
...
out.append(
    f"insert into bewertungsschluessel (id,name,stufen,bestehensgrenze) values ({q(SCHLUESSEL)},{q('IHK 100-Punkte-Schlüssel')},{jsonb(stufen)},50) on conflict (id) do nothing;"
)
