...
out = ["-- AUTOGENERIERT von scripts/generate_seed.py — nicht von Hand ändern.",
       "-- Quelle: " + ", ".join(p.name for p in POOLS),
       "begin;\n"]

out.append(f"insert into traeger (id,name,slug) values ({q(TRAEGER)},"
...
KAMMERN = {
    ...
    "HWK": (uid("kammer", "hwk-aachen"), "Handwerkskammer Aachen", "HWK", "Sandkaulbach 17-21, 52062 Aachen"),
}
for kid, name, typ, anschrift in KAMMERN.values():
    out.append(f"insert into kammern (id,name,typ,anschrift) values ({q(kid)},{q(name)},"
...
out.append(f"insert into bewertungsschluessel (id,name,stufen,bestehensgrenze) values "
